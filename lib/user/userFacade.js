...
function createRoom(raw_details){
    return usrService.createRoom(raw_details.user.userId,raw_details.body)
    .then((result)=>{
        if(result){

            return userMapper.roomCreated(result)
        }else{
            return userMapper.roomAlreadyExist();
        }
    });  
}
...
/**
 * @function checking current rooms
 */

function viewRooms(user){
    return usrService.viewRooms(user)
        .then(result=>userMapper.viewRooms(result));
}


/**
 * @function checking all users
 */

function viewAll(id){
    return usrService.viewAll(id)
        .then(result=>userMapper.viewRooms(result));
}
...
function getChat(id){
    return usrService.getChat(id)
        .then(result=>userMapper.viewChat(result));
}
//========================== Export Module Start ==============================
